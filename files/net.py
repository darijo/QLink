...
class Mininet(object):
    ...
    def addQlink(self, node1, node2, params1, params2, qspec1=None, qspec2=None, **params):
        ...
        if ('bw' in params1):
            bw1 = params1['bw']
            if (bw1 <= 0 or bw1 > bwParamMax):
                error('Bandwidth limit for bw1: ', bw1, 'is outside supported range 0..%d' % bwParamMax, '- setting to max value\n' )
                bw1 = bwParamMax
            modifiedParams1 = params1
            modifiedParams1.pop('bw')
        else:
            bw1 = bwParamMax
        if ('bw' in params2):
            bw2 = params2['bw']
            if (bw2 <= 0 or bw2 > bwParamMax):
                error('Bandwidth limit for bw2: ', bw2, 'is outside supported range 0..%d' % bwParamMax, '- setting to max value\n' )
            modifiedParams2 = params2
            modifiedParams2.pop('bw')
        else:
            bw2 = bwParamMax

        addlink1 = self.addLink(node1, st, cls=TCULink, params1={}, params2=modifiedParams2)
        if str(addlink1.intf1)[0] == 's':    # if it is a switch, Install queues   [Zahran] - recommend generalize for s or S
            ...
            self.qparams[addlink1.intf1] = [qspec1, bw1]

        addlink2 = self.addLink(st, node2, cls=TCULink, params1=modifiedParams1, params2={})
